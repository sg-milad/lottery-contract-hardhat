...
describe("Lottery", function () {
  ...
  describe("Deployment", async () => {
    it("Should set the right owner(manager)", async () => {
      ...
      expect(await lottery.read.manager()).to.equal(getAddress(manager.account.address));
    });
  });
  ...
  describe("enterPlayer", async () => {
    ...
    it("Should fail if the ether sent is Insufficient", async () => {
      ...
      const enterPlayer = lottery.write.enterPlayer({ value: parseEther("0.001"), account: AccountOne.account })
      await expect(enterPlayer).to.be.rejectedWith("Insufficient ether sent")
    })
    it("Should set right player", async () => {
      const { lottery, AccountOne } = await loadFixture(deployLotteryFixture);
      await lottery.write.enterPlayer({ value: parseEther("200"), account: AccountOne.account.address })
      const player = (await lottery.read.getPlayers()).at(0)
      expect(player?.toLocaleLowerCase()).to.be.equal(AccountOne.account.address)
    })
  })
  ...
  describe("pickWinner", async () => {
    ...
    it("should fail if other accounts try to call pickWinner function", async () => {
      const { lottery, AccountOne, } = await loadFixture(deployLotteryFixture);
      const pickWinner = lottery.write.pickWinner({ account: AccountOne.account.address })
      await expect(pickWinner).to.be.rejectedWith("Only the manager can call this function")
    })
    it("should pick random winner", async () => {
      const { lottery, AccountOne, accountTwe, manager, } = await loadFixture(deployLotteryFixture);
      await lottery.write.enterPlayer({ value: parseEther("2000"), account: AccountOne.account })
      await lottery.write.enterPlayer({ value: parseEther("2000"), account: accountTwe.account })
      expect(await lottery.write.pickWinner({ account: manager.account })).to.be.ok
    })
    it("sends money to the winner and resets the players array", async () => {
      const { lottery, AccountOne, publicClient, manager } = await loadFixture(deployLotteryFixture);
      const beforeTransfer = await publicClient.getBalance({ address: AccountOne.account.address })
      await lottery.write.enterPlayer({ value: parseEther("2000"), account: AccountOne.account })
      await lottery.write.pickWinner({ account: manager.account })
      const afterTransfer = await publicClient.getBalance({ address: AccountOne.account.address })
  ...
  describe("getPlayers", async () => {
    it("players array length should be one ", async () => {
      const { lottery, AccountOne } = await loadFixture(deployLotteryFixture);
      await lottery.write.enterPlayer({ value: parseEther("200"), account: AccountOne.account })
